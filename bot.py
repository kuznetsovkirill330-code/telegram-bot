...
async def back(message: Message, state: FSMContext):
    ...
    if message.from_user.id in active_manager_ticket:
        active_manager_ticket.pop(message.from_user.id)
    await message.answer("Главное меню:", reply_markup=main_kb(message.from_user.id))

# ================= МЕНЮ =================

@dp.message(F.text == "🍽 Меню")
async def show_menu_days(message: Message):
...
@dp.message(F.text.in_(menu_data.keys()))
async def show_day_menu(message: Message):
...
@dp.message(F.text.in_(["🚫 Проблема", "💡 Предложение"]))
